...
유니온 타입은 명료하고 간단한 케이스 뿐 아니라 여러 경우에 쓴다다.  
```
type FormEvent =
...
...
```
FormEvent는 서로 다른 이벤트라서 정밀한 타입 검사로 케이스를 좁혀야 이벤트 속성을 바르게 쓸 수 있다.  
...
외부에 뭔가를 요청했을 때 보통은 유니온 타입으로 응답을 예상한다.  
그러나 유니온 타입으로도 장담하기 어려운 경우, 우선 unknown으로 지정한 다음 타입을 좁혀나가는 방법을 권장한다.  
```
type Result =
